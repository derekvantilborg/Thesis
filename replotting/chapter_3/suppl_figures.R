...
figure_s1 = function(df){
  ...
  mean_algo = df_classical %>% group_by(algorithm) %>% dplyr::summarise_all("mean")
  mean_descr = df_classical %>% group_by(descriptor) %>% dplyr::summarise_all("mean")
  ...
  box_plot_classical = ggplot(df_classical, aes(x=algorithm, y=rmse, fill = descriptor))+
    geom_jitter(aes(fill=descriptor), position=position_jitterdodge(0), size=1, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    geom_boxplot(alpha=0.1, outlier.size = 0, position = position_dodge(0.75), width = 0.50,
                 outlier.shape=NA, varwidth = FALSE, lwd=0.35, fatten=0.75) +
  ...
  box_plot_dl = ggplot(df_dl, aes(x=algorithm, y=rmse, fill=descriptor))+
    geom_jitter(aes(fill=descriptor), position=position_jitterdodge(0), size=1, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    geom_boxplot(alpha=0.1, outlier.size = 0, position = position_dodge(0.75), width = 0.275,
                                outlier.shape=NA, varwidth = FALSE, lwd=0.35, fatten=0.75) +
    scale_y_continuous(breaks = seq(0.25,1.75,0.25), limits = c(0.25,1.75), expand = expansion(mult = c(0.01, 0.01)))+
    coord_cartesian(ylim=c(0.25, 1.75))+
...
figure_s2 = function(df){
  ...
  pca_plot_ml = ggplot(bi_all_ml, aes(x = x, y =y)) +
    ...
    geom_point(aes(x, y ), fill='#101e25', size=1.25, shape=21, alpha=ifelse(bi_all_ml$type == 'Score', 0, 0.8), color = "black", stroke = 0.1) +
    ...
    labs(x = paste0('PC1 (',round(pca_all_ml$scree$data$eig[1],1),'%)'),
         y = paste0('PC2 (',round(pca_all_ml$scree$data$eig[2],1),'%)'),
  ...
  pca_plot_dl = ggplot(bi_all_dl, aes(x = x, y =y)) +
    ...
    geom_text_repel(aes(label = target_name), colour = bi_all_dl$col, alpha = ifelse(bi_all_dl$type == 'Score', 0, 1), 
                    size = 2, segment.size = 0.25, force = 20, fontface="bold", max.iter = 1505, 
                    max.overlaps = 30, show.legend = FALSE)+
    labs(x = paste0('PC1 (',round(pca_all_dl$scree$data$eig[1],1),'%)'),
         y = paste0('PC2 (',round(pca_all_dl$scree$data$eig[2],1),'%)'),
         shape = 'Algorithm',  color = 'Descriptor') +
    guides(shape = guide_legend(override.aes = list(color  = "#101e25"), order = 2),
           color = guide_legend(override.aes = list(shape = 16), order = 1 )) +
    default_theme
...
figure_s3 = function(df){
  ...
  scatter_mini_plot = function(data, algo, descr, xlab='', ylab='', label=''){
    ...
    ggplot(data, aes(x=rmse, y=cliff_rmse, fill=algorithm))+
      geom_point(alpha = ifelse(data$algorithm == algo & data$descriptor == descr, 0, 0.2), size=1.25, shape=21, color = "#b6b6b6", stroke = 0.1, fill='#b6b6b6')+
      geom_point(alpha = ifelse(data$algorithm == algo & data$descriptor == descr, 0.8, 0), size=1.25, shape=21, color = "black", stroke = 0.1)+
...
figure_s4 = function(df){
  ...
  corrplot(wc$p.value, p.mat = wc$p.value, is.corr=F, col="#FFFFFF", type = 'lower', tl.col = 'black', sig.level = c(0.05), method = 'circle', cl.pos = 'n', pch = "*", col.lim = c(0, 1), insig = 'label_sig',
           tl.srt = 45, pch.cex = 1.4, tl.cex = 0.6)
...
figure_s5 = function(df){
  ...
  df$perc_test_cliffs = datasets$`Percentage test cliffs`[match(df$dataset, datasets$Dataset)]
  ...
  rmse_per_dataset = dplyr::summarise(rmse_per_dataset, mean_cliff_rmse=mean(cliff_rmse), min_cliff_rmse=min(cliff_rmse),
  ...
  p_datasize = ggplot(rmse_per_dataset, aes(y = mean_cliff_rmse, x = train_size))+
    geom_errorbar(aes(ymin=min_cliff_rmse, ymax=max_cliff_rmse),
                  colour='#101e25', width=0, size=0.5, alpha=0.5)+
    geom_point(fill='#101e25', size=1.25, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
  ...
  ### B ###
  rmse_per_perc_cliff <- group_by(df, dataset)
  rmse_per_perc_cliff = dplyr::summarise(rmse_per_perc_cliff, 
                                         mean_rmse=mean(rmse), 
                                         min_rmse=min(rmse),
                                         max_rmse=max(rmse), 
                                         perc_test_cliffs=mean(perc_test_cliffs))
  
  p_rmse_per_perc_cliff = ggplot(rmse_per_perc_cliff, aes(y = mean_rmse, x = perc_test_cliffs))+
    geom_errorbar(aes(ymin=min_rmse, ymax=max_rmse),
                  colour='#101e25', width=0, size=0.5, alpha=0.5)+
    geom_point(fill='#101e25', size=1.25, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    labs(x="% activity cliff compounds (test)", y="RMSE") +
    scale_y_continuous(breaks = seq(0.25,1.75,0.25), limits = c(0.01,1.75), expand = expansion(mult = c(0.01, 0.01)))+
    scale_x_continuous(breaks = seq(0,55,10), limits = c(0,55), expand = expansion(mult = c(0.01, 0.01))) +
    coord_cartesian(ylim=c(0.25, 1.75))+
    scale_color_manual(values = colours)+
    guides(fill = 'none', color = 'none')+
    default_theme
  ...
  ### C ###
  cliff_rmse_per_perc_cliff <- group_by(df, dataset)
  cliff_rmse_per_perc_cliff = dplyr::summarise(cliff_rmse_per_perc_cliff, 
                                               mean_cliff_rmse=mean(cliff_rmse), 
                                               min_cliff_rmse=min(cliff_rmse),
                                               max_cliff_rmse=max(cliff_rmse), 
                                               perc_test_cliffs=mean(perc_test_cliffs))
  
  p_cliff_rmse_per_perc_cliff = ggplot(cliff_rmse_per_perc_cliff, aes(y = mean_cliff_rmse, x = perc_test_cliffs))+
    geom_errorbar(aes(ymin=min_cliff_rmse, ymax=max_cliff_rmse),
                  colour='#101e25', width=0, size=0.5, alpha=0.5)+
    geom_point(fill='#101e25', size=1.25, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    labs(x="% activity cliff compounds (test)", y=bquote("RMSE"[cliff])) +
    scale_y_continuous(breaks = seq(0.25,1.75,0.25), limits = c(0.01,1.75), expand = expansion(mult = c(0.01, 0.01)))+
    scale_x_continuous(breaks = seq(0,55,10), limits = c(0,55), expand = expansion(mult = c(0.01, 0.01))) +
    coord_cartesian(ylim=c(0.25, 1.75))+
    scale_color_manual(values = colours)+
  ...
  ### D ###
  delta_rmse_per_perc_cliff <- group_by(df, dataset)
  delta_rmse_per_perc_cliff = dplyr::summarise(delta_rmse_per_perc_cliff, 
                                               mean_delta_rmse=mean(cliff_rmse - rmse), 
                                               min_delta_rmse=min(cliff_rmse - rmse),
                                               max_delta_rmse=max(cliff_rmse - rmse), 
                                               perc_test_cliffs=mean(perc_test_cliffs))
  
  p_delta_rmse_per_perc_cliff = ggplot(delta_rmse_per_perc_cliff, aes(y = mean_delta_rmse, x = perc_test_cliffs))+
    geom_errorbar(aes(ymin=min_delta_rmse, ymax=max_delta_rmse),
                  colour='#101e25', width=0, size=0.5, alpha=0.5)+
    geom_point(fill='#101e25', size=1.25, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    labs(x="% activity cliff compounds (test)", y=bquote("RMSE"[cliff]~ "- RMSE")) +
    scale_y_continuous(breaks = seq(-0.2, 0.6, 0.2), limits = c(-0.2,0.6), expand = expansion(mult = c(0.01, 0.01)))+
    scale_x_continuous(breaks = seq(0,55,10), limits = c(0,55), expand = expansion(mult = c(0.01, 0.01))) +
    geom_hline(yintercept = 0, linetype = 'dashed', alpha = 0.25) +
  ...
  colours = descr_cols$cols[match(c('WHIM', 'Graph', 'Physchem', 'SMILES', 'MACCs', 'ECFP'), descr_cols$descr)]
  
  p_drug_target = ggplot(df, aes(x=receptor, y=cliff_rmse, fill = descriptor))+
    # geom_jitter(aes(color=descriptor), position=position_jitterdodge(0), size=0.5, shape=1, alpha=0.5) +
    # geom_jitter(aes(color=descriptor), position=position_jitterdodge(0), size=0.5, shape=19, alpha=0.5) +
    
    geom_jitter(aes(fill=descriptor), position=position_jitterdodge(0), size=1, shape=21, alpha=0.8, color = "black", stroke = 0.1) +
    # geom_boxplot(alpha=0.5, outlier.size = 0, position = position_dodge(0.75), width = 0.35, outlier.shape=NA, varwidth = FALSE, lwd=0.3, fatten=0.75) +
  ...
  ps5_abcd = plot_grid(p_datasize, p_rmse_per_perc_cliff, p_cliff_rmse_per_perc_cliff, p_delta_rmse_per_perc_cliff,
            ncol=4, nrow =1, labels=c('a', 'b', 'c', 'd'), label_size = 10)
  
  ps5_e = plot_grid(p_drug_target, plot_spacer() + theme(panel.background = element_blank()), labels=c('e', ''), label_size = 10, rel_widths = c(0.9, 0.1))
  
  ps5 = plot_grid(ps5_abcd,
                  ps5_e,
                  ncol=1, nrow =2)
...
dev.print(pdf, 'sup_fig_1.pdf', width = 5.12, height = 1.5)

print(figure_s2(benchmark))
dev.print(pdf, 'sup_fig_2.pdf', width = 5.12, height = 1.5)

print(figure_s3(benchmark))
dev.print(pdf, 'sup_fig_3.pdf', width = 5.12, height = 4.8)

print(figure_s4(benchmark))
dev.print(pdf, 'sup_fig_4.pdf', width = 5.12, height = 5.12)

print(figure_s5(benchmark))
dev.print(pdf, 'sup_fig_5.pdf', width = 5.12, height = 2.35)
