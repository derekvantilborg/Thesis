...
df <- read_csv("data/processed_results.csv")
...
sfig1a = ggplot(dfs1a, aes(x = batch_size, y = enrichment, color = acquisition_method, fill = acquisition_method))+
  ...
  labs(y = 'Mean enrichment in\n1000 acquired molecules', x='Molecules acquired per cycle') +
...
sfig1c = ggplot(dfs1c, aes(x = batch_size, y = test_roc_auc, color = acquisition_method, fill = acquisition_method))+
  ...
  scale_color_manual(values=custom_colours) +
  scale_fill_manual(values=custom_colours) +
  default_theme + theme(plot.margin = unit(c(0.25, 0, 0.25, 0.15), "cm"))
...
pdf('sfig1.pdf', width = 5.118, height = 1.2)
print(sfig1)
dev.off()
...
umap_aldh1 <- read_csv("data/UMAP_ALDH1.csv")
# umap_aldh1 = subset(umap_aldh1, split == 's')
umap_aldh1 = umap_aldh1[order(umap_aldh1$y),]
umap_aldh1$size = umap_aldh1$y
umap_aldh1$size[umap_aldh1$size == 1] = 0.5
umap_aldh1$size[umap_aldh1$size == 0] = 1

p_umap_aldh1 = ggplot(umap_aldh1, aes(x=UMAP1, y=UMAP2, color = as.character(y), fill = as.character(y)))+
...
umap_pkm2$size[umap_pkm2$size == 1] = 0.5
umap_pkm2$size[umap_pkm2$size == 0] = 1

p_umap_pkm2 = ggplot(umap_pkm2, aes(x=UMAP1, y=UMAP2, color = as.character(y), fill = as.character(y)))+
  scale_color_manual(values=c("#cccccc", "black")) +
  scale_fill_manual(values=c("#cccccc", "#0d677c")) +
  geom_point(size=umap_aldh1$size, alpha=0.5, shape=21, stroke=0.1) +
  theme(
...
umap_vdr$size[umap_vdr$size == 1] = 0.5
umap_vdr$size[umap_vdr$size == 0] = 1

p_umap_vdr = ggplot(umap_vdr, aes(x=UMAP1, y=UMAP2, color = as.character(y), fill = as.character(y)))+
  scale_color_manual(values=c("#cccccc", "black")) +
  scale_fill_manual(values=c("#cccccc", "#0d677c")) +
...
pdf('sfig2.pdf', width = 5.118, height = 1.7)
print(sfig2)
dev.off()
