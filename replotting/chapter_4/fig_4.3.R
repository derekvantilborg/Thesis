...
colours = c('#0d677c', '#72af9c', '#ffb361', '#B6BEC2', '#1e3648', '#7F9ED2')
...
descr_cols = list(cols = c('#0d677c','#83b0b7','#f76c3c','#ffb361','#ffffff', '#098e91', '#72af9c',
                           '#101e25', '#101e25'),
...
setwd("/Users/derekvantilborg/Dropbox/PhD/Thesis/replotting/chapter_4")

# load data
all_samples <- read.csv("data/all_samples.csv")
...
df_gh = na.omit(df_gh)
df_gh$cycle_type = gsub('DOE', '0', df_gh$cycle_type)
df_gh$cycle_type = gsub('Exploration', '1', df_gh$cycle_type)
df_gh$cycle_type = gsub('Exploitation', '2', df_gh$cycle_type)
df_gh$cycle_type = factor(df_gh$cycle_type, levels=c("0", "1", "2"))
...
library_doe <- read_csv("results/screen_predictions_0_5Apr.csv")
library_doe <- library_doe[order(library_doe$ID), ]

library_explore <- read_csv("results/screen_predictions_0_5Apr.csv")
library_explore <- library_explore[order(library_explore$ID), ]

library_exploit <- read_csv("results/screen_predictions_1_19Apr.csv")
library_exploit <- library_exploit[order(library_exploit$ID), ]

library_val <- read_csv("results/screen_predictions_2_24Apr.csv")
...
p_4a1 = ggplot(pca_df_doe, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  labs(x=paste0('PC1 (', round(summ_doe$importance[2,1]*100, 2), '%)'),  y=paste0('PC2 (', round(summ_doe$importance[2,2]*100,2), '%)'))+
  geom_point(alpha=0.75, size=0.5, shape=16) + 
  # geom_point(data=subset(pca_df_doe, pick != 'None'), aes(x = PC1, y=PC2), size = 0.5, color='#36596D', fill='#3a5f75', shape=21, alpha=1) +
  geom_point(data=subset(pca_df_doe, pick != 'None'), aes(x = PC1, y=PC2), size = 1, alpha=1, stroke = 0.1, color='black', shape=21, fill='#0d677c') +
  coord_cartesian(xlim = c(-4, 3.25), ylim = c(-4, 3.25)) +
  scale_color_gradient(low = "lightgrey", high = "lightgrey", na.value = NA) +
  custom_theme +
  theme(plot.margin = unit(c(0.25, 0.25, 0.25, 0), "cm"))
...
p_4a2 = ggplot(pca_df_explore, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  labs(x=paste0('PC1 (', round(summ_explore$importance[2,1]*100, 2), '%)'),  y=paste0('PC2 (', round(summ_explore$importance[2,2]*100,2), '%)'))+
  geom_point(alpha=0.75, size=0.5, shape=16) + 
  geom_point(data=subset(pca_df_explore, pick != 'None'), aes(x = PC1, y=PC2), size = 1, alpha=1, stroke = 0.1, color='black', shape=21, fill='#72af9c') +
  coord_cartesian(xlim = c(-4, 3.25), ylim = c(-4, 3.25)) +
...
p_4a3 = ggplot(pca_df_exploit, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  ...
  geom_point(data=subset(pca_df_exploit, pick != 'None'), aes(x = PC1, y=PC2), size = 1, alpha=1, stroke = 0.1, color='black', shape=21, fill='#ffb361') +
  scale_color_gradient(low = "darkgrey", high = "white", na.value = NA) +
  custom_theme +
  theme(plot.margin = unit(c(0.25, 0.25, 0.25, 0), "cm"))
...
p_4b = ggplot(df_gh, aes(x = cycle_type, y = Uptake, fill = as.character(cycle), color = as.character(cycle))) +
  ...
  geom_boxplot(width=0.3, alpha=0.5, lwd=0.3, fatten=0.75, varwidth = FALSE) +
...
p_4c = ggplot(df_gh, aes(x = ID2, y = Uptake, fill = cycle, color = as.character(cycle))) +
  ...
  scale_y_continuous(breaks = seq(0, 15, 5), limits = c(0,16), expand = expansion(mult = c(0.0, 0.0))) +
  geom_errorbar(aes(ymin=Uptake-Uptake_stdev, ymax=Uptake+Uptake_stdev), width=0.4, position=position_dodge(0.75), size=0.35, color='black') +
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
...
df_gh_long1$variable = gsub("PLGA", 'x1', df_gh_long1$variable)
df_gh_long1$variable = gsub("PP.L", 'x2', df_gh_long1$variable)
df_gh_long1$variable = gsub("PP.COOH", 'x3', df_gh_long1$variable)
df_gh_long1$variable = gsub("PP.NH2", 'x4', df_gh_long1$variable)
df_gh_long1$variable = gsub("S.AS", 'x5', df_gh_long1$variable)
df_gh_long1$variable = factor(df_gh_long1$variable, levels = rev(c("x1", "x2", "x3", "x4", "x5")))

# scale the values a bit to visuzalize them better
df_gh_long1$value2 = df_gh_long1$value*2.5
# df_gh_long1$variable = paste0('x', df_gh_long1$variable)
...
p_4cd = plot_grid(p_4c, p_4d, ncol=1, labels = c('c', 'd'), label_size=10, 
                  rel_heights = c(7, 10))


##### Fig 4. ####

fig4_upper = plot_grid(p_4a1, p_4a2, p_4a3, p_4b, ncol=4, labels = c('a', '', '', 'b'), label_size=10, rel_widths = c(1,1,1,0.8))
fig_4 = plot_grid(fig4_upper, p_4cd, ncol=1, labels = c('', ''), label_size=10, rel_heights = c(2,3))
fig_4 = plot_grid(plot_spacer(), fig_4, rel_widths = c(0.01, 1))
fig_4


dev.print(pdf, 'Fig_4.3.pdf', width = 5.118, height = 3.5) # width = 7.205, height = 4
...
p_5a = ggplot(pca_df_val, aes(x=PC1, y=PC2, color=log10(y_uncertainty_uptake))) +
  ...
  geom_point(alpha=0.75, size=0.5, shape=16) + 
  coord_cartesian(xlim = c(-4, 3.25), ylim = c(-4, 3.25)) +
  geom_point(data=subset(pca_df_val, pick != 'None'), aes(x = PC1, y=PC2, shape=type, fill=type), size = 1, color='black', alpha=1, stroke = 0.1) +
  scale_color_manual(values=c('#9dc6c6', '#098e91')) +
  scale_fill_manual(values=c('#9dc6c6', '#098e91')) +
  scale_shape_manual(values=c(24, 21)) +
  scale_color_gradient(low = "darkgrey", high = "white", na.value = NA) +
  custom_theme + 
  theme(plot.margin = unit(c(0.25, 0.5, 0.25, 0), "cm"))
...
p_5b = ggplot(df5_b, aes(y=ID2, x=Uptake, group=type, fill=type))+
  ...
  scale_color_manual(values=c('lightgrey', '#9dc6c6', "#098e91")) +
  scale_fill_manual(values=c('lightgrey', '#9dc6c6', "#098e91")) +
...
df5_c_long = melt(df5_b, id = c('ID2', "cycle_type"))
...
df5_c_long$variable = gsub("PLGA", 'x1', df5_c_long$variable)
df5_c_long$variable = gsub("PP.L", 'x2', df5_c_long$variable)
df5_c_long$variable = gsub("PP.COOH", 'x3', df5_c_long$variable)
df5_c_long$variable = gsub("PP.NH2", 'x4', df5_c_long$variable)
df5_c_long$variable = gsub("S.AS", 'x5', df5_c_long$variable)

df5_c_long$variable = factor(df5_c_long$variable, levels = rev(c("x1", "x2", "x3", "x4", "x5")))
df5_c_long$value = as.numeric(df5_c_long$value)
df5_c_long$value2 = df5_c_long$value*2.5
...
p_5c = ggplot(df5_c_long, aes(y = ID2, x=variable, color=cycle_type, size=value))+
  ...
  custom_theme  +
  theme(plot.margin = unit(c(0.175, 0.25, 0.25, -0.8), "cm"),
...
sup_p1a = ggplot(subset(df_gh_long, variable %in% c("Z_ave")), aes(x = ID2, y=value, fill= as.character(cycle)))+
  geom_bar(stat="identity", color="black", position=position_dodge(), width = 0.75, size=0.25) +
  geom_errorbar(aes(ymin=value-Z_ave_stdev, ymax=value+Z_ave_stdev), width=0.4, position=position_dodge(0.75), size=0.35, color='black') +
  scale_color_manual(values=colours) +
...
sup_p1b = ggplot(subset(df_gh_long, variable %in% c("PDI")), aes(x = ID2, y=value, fill= as.character(cycle)))+
  geom_bar(stat="identity", color="black", position=position_dodge(), width = 0.75, size=0.25) +
  geom_errorbar(aes(ymin=value-PDI_stdev, ymax=value+PDI_stdev), width=0.4, position=position_dodge(0.75), size=0.35, color='black') +
...
sup_p1 = plot_grid(sup_p1a, sup_p1b, ncol=1, labels = c('a', 'b'), label_size=8, rel_heights = c(2,3))
sup_p1

dev.print(pdf, 'sFig_4.1.pdf', width = 5.118, height = 2)
...
ps1_a1 = ggplot(subset(all_samples, cycle == 0  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Uptake, y = Uptake_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_errorbar(aes(ymin=Uptake_pred_5, ymax=Uptake_pred_95), width=.4, alpha=0.5, size=0.3) +
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 1  & pick_for_next_cycle == 'yes'), aes(x = Uptake, y = Uptake_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
...
ps1_a2 = ggplot(subset(all_samples, cycle == 0  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = PDI, y = PDI_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(data=subset(all_samples, cycle == 1  & pick_for_next_cycle == 'yes'), aes(x = PDI, y = PDI_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
  scale_shape_manual(values=c(21, 24)) +
...
ps1_a3 = ggplot(subset(all_samples, cycle == 0  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Z_ave, y = Z_ave_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 1  & pick_for_next_cycle == 'yes'), aes(x = Z_ave, y = Z_ave_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
  scale_shape_manual(values=c(21, 24)) +
...
ps1_b1 = ggplot(subset(all_samples, cycle == 1  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Uptake, y = Uptake_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_errorbar(aes(ymin=Uptake_pred_5, ymax=Uptake_pred_95), width=.4, alpha=0.5, size=0.3) +
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 2  & pick_for_next_cycle == 'yes'), aes(x = Uptake, y = Uptake_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
...
ps1_b2 = ggplot(subset(all_samples, cycle == 1  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = PDI, y = PDI_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 2  & pick_for_next_cycle == 'yes'), aes(x = PDI, y = PDI_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
  scale_shape_manual(values=c(21, 24)) +
  coord_cartesian(xlim = c(0.05, 0.2), ylim = c(0.05, 0.2)) +
...
ps1_b3 = ggplot(subset(all_samples, cycle == 1  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Z_ave, y = Z_ave_pred, color = cycle_origin, fill = cycle_origin))+
  ...
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 2  & pick_for_next_cycle == 'yes'), aes(x = Z_ave, y = Z_ave_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  scale_color_manual(values=colours) +
  scale_fill_manual(values=colours) +
  scale_shape_manual(values=c(21, 24)) +
  coord_cartesian(xlim = c(50, 300), ylim = c(50, 300)) +
...
ps1_c1 = ggplot(subset(all_samples, cycle == 2  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Uptake, y = Uptake_pred, color = cycle_origin, fill = cycle_origin))+
  labs(x='Measured uptake (fold)', y='Predicted uptake (fold)') +
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_errorbar(aes(ymin=Uptake_pred_5, ymax=Uptake_pred_95), width=.3, alpha=0.5, size=0.25) +
  geom_point(size=1.5, alpha=1, stroke = 0.1, color='black')+
  geom_point(data=subset(all_samples, cycle == 3  & pick_for_next_cycle == 'yes'), aes(x = Uptake, y = Uptake_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  ...
  scale_fill_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  scale_color_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  coord_cartesian(xlim = c(-1, 15), ylim = c(-1, 15)) +
  scale_shape_manual(values=c(21, 24)) +
  custom_theme

ps1_c2 = ggplot(subset(all_samples, cycle == 2  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = PDI, y = PDI_pred, fill = cycle_origin))+
  labs(x='Measured PDI (µm)', y='Predicted PDI (µm)') +
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(size=1.5, alpha=1, stroke = 0.1, color='black')+
  geom_point(data=subset(all_samples, cycle == 3  & pick_for_next_cycle == 'yes'), aes(x = PDI, y = PDI_pred), color='black', size=1.5, alpha=1, stroke = 0.1)+
  scale_fill_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  scale_color_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  coord_cartesian(xlim = c(0.05, 0.2), ylim = c(0.05, 0.2)) +
  scale_shape_manual(values=c(21, 24)) +
...
ps1_c3 = ggplot(subset(all_samples, cycle == 2  & pick_for_next_cycle == 'no'), aes(shape = pick_for_next_cycle, x = Z_ave, y = Z_ave_pred, fill = cycle_origin))+
  labs(x='Measured size (nm)', y='Predicted size (nm)') +
  geom_abline(slope=1, linetype='dashed', alpha=0.25, color='#101e25')+
  geom_point(size = 1.5, color='black', alpha=1, stroke = 0.1)+
  geom_point(data=subset(all_samples, cycle == 3  & pick_for_next_cycle == 'yes'), aes(x = Z_ave, y = Z_ave_pred), size=1.5, alpha=1, stroke = 0.1)+
  scale_fill_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  scale_color_manual(values=c('#3a5f75', '#81b3b0', '#EABA6B', '#098e91')) +
  scale_shape_manual(values=c(21, 24)) +
  coord_cartesian(xlim = c(50, 300), ylim = c(50, 300)) +
...
ps1a = plot_grid(ps1_a1, ps1_a2, ps1_a3, ncol=3, labels = c('a', 'b', 'c'), label_size=10)
ps1b = plot_grid(ps1_b1, ps1_b2, ps1_b3, ncol=3, labels = c('d', 'e', 'f'), label_size=10)
ps1c = plot_grid(ps1_c1, ps1_c2, ps1_c3, ncol=3, labels = c('g', 'h', 'i'), label_size=10)

plot_grid(ps1a, ps1b, ps1c, ncol=1)

dev.print(pdf, 'sFig_4.2.pdf', width = 5.118, height = 4.8)
...
p_uptake_plga = ggplot(library_val, aes(x=x_PLGA, y=y_hat_uptake, color = y_uncertainty_uptake))+
  ...
  scale_y_continuous(breaks = seq(0, 15, 5), limits = c(0,16), expand = expansion(mult = c(0.0, 0.0))) +
  scale_color_gradientn(colors = c("#113959","#0d677c" , '#72af9c', '#EABA6B')) +
  geom_point(alpha=0.75, size=0.5, shape=16) +  custom_theme +
  theme(
    plot.margin = unit(c(0.1, 0, 0.1, 0.25), "cm")
    )
...
p_uptake = plot_grid(p_uptake_plga, p_uptake_peg, p_uptake_cooh, p_uptake_nh2, p_uptake_sas, ncol=5, rel_widths = c(2.45,2,2,2,3))
p_pdi = plot_grid(p_pdi_plga, p_pdi_peg, p_pdi_cooh, p_pdi_nh2, p_pdi_sas, ncol=5, rel_widths = c(2.45,2,2,2,3))
p_size = plot_grid(p_size_plga, p_size_peg, p_size_cooh, p_size_nh2, p_size_sas, ncol=5, rel_widths = c(2.45,2,2,2,3))

p_rela = plot_grid(p_uptake, p_pdi, p_size, ncol=1, labels = c('a', 'b', 'c'), label_size=10)
p_rela

dev.print(pdf, 'sFig_4.3.pdf', width = 5.118, height = 3.2)
...
# 
# 
dfs4_bnn <- read.csv("results/uptake_model_0_eval_bnn_5Apr.csv")
dfs4_rf <- read.csv("results/uptake_model_0_eval_rf_28May.csv")
dfs4_xgb <- read.csv("results/uptake_model_0_eval_xgb_28May.csv")
dfs4_gp <- read.csv("results/uptake_model_0_eval_gp_28May.csv")
# 
bnn_rmse = 1.227
rf_rmse = 1.197
xgb_rmse = 1.185
...
ps4_a = ggplot(dfs4_bnn, aes(x = y, y = y_hat))+
  ...
  geom_point(size=1.5, alpha=1, fill = '#3a5f75', stroke = 0.1, color='black', shape=21,)+
  scale_color_manual(values=colours) +
...
dev.print(pdf, 'sFig_4.4.pdf', width = 5.118, height = 1.25)

# 
# ##### S5. Design space homogeneity ####
# 
# # Design space homogeneity
design_dist <- read.csv("data/mean_dist_designspace.csv")
design_dist = na.omit(design_dist)
design_dist$k = factor(design_dist$k)
# 
ps5 = ggplot(design_dist, aes(x = mean_dist, color=k, fill=k)) +
  # geom_histogram(binwidth=0.005, fill='#B6BEC2') +
  geom_density(alpha=0.8) +
  labs(x = 'Mean Euclidean distance to k closest points', y='Density') +
  scale_color_manual(values=c('#3a5f75', '#72af9c')) +
  scale_fill_manual(values=c('#3a5f75', '#72af9c')) +
  custom_theme +
  theme(legend.position = 'none', legend.direction="vertical",
        legend.title = element_text(size=7, face="plain", colour = "#101e25"),
        legend.text = element_text(size=7, face="plain", colour = "#101e25"),
        axis.text.y.left = element_blank(),
        axis.ticks.y = element_blank())

ps5 = plot_grid(plot_spacer(), ps5, plot_spacer(), ncol=3, rel_widths = c(1,2,1))
ps5
# 
dev.print(pdf, 'sFig_4.5.pdf', width = 5.118, height = 1.2)
